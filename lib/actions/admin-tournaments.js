...
export async function generateTournamentBracket(id) {
  try {
    ...
    const keyToId = Object.fromEntries(
      built.matches.map((m) => [m.tempKey, randomUUID()])
    );

    const matchRecords = built.matches.map((m) => ({
      id: keyToId[m.tempKey],
      tournamentId: id,
      round: m.round,
      ...
      nextMatchId: m.nextMatchKey ? keyToId[m.nextMatchKey] : null,
      nextSlot: m.nextSlot,
      loserNextMatchId: m.loserNextMatchKey
        ? keyToId[m.loserNextMatchKey]
        : null,
    ...
    await prisma.$transaction(
      async (tx) => {
        ...
        await tx.tournamentMatch.createMany({ data: matchRecords });
